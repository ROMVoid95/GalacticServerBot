...
public class GalacticEventListener extends ListenerAdapter
{
    ...
    public void onMessageReceived(MessageReceivedEvent event)
    {
        if(event.getChannelType().isGuild())
        {
            if (!GalacticBot.instance().isDevBot())
            {
                ...
                {
                    var	options	= BotData.database().galacticBot().getSuggestionOptions();
                    MessageChannel		channel	= event.getChannel();

                    if (options.getSuggestionsChannelId().equals(channel.getId()))
                    {
                        var type = event.getMessage().getType();
    ...
    {
        LogUtils.log(event.getGuild().getDefaultChannel().createInvite().complete().getUrl());
    }
}
